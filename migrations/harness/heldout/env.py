"""Alembic environment for the `heldout` schema.

Protected set (docs/tier4/protected-paths-policy.md). Reference values and
perturbations, reachable by `alfred_criterion` alone and by SELECT alone — it cannot
write the answers it reads.

**Additive-only, at the row level as well as the schema level.** A corrected
reference value is a new row under a new version, never an UPDATE: otherwise a verdict
computed last week cannot be reproduced, and reproducibility is the product.
`downgrade()` must raise, as in `evidence`.
"""
...
SCHEMA = "heldout"
ROLE_ENV_VAR = "ALFRED_DB_URL_MIGRATOR_HELDOUT"
VERSION_TABLE = "alembic_version_heldout"

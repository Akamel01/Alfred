...
| C14 | End-of-run re-assertion of C7, C9, C12, C13, C16, compared **by observed value and not only by outcome** — a mount whose mode moved, a writable root that grew, a container swapped for another of the same kind, and an observation a check stopped making | inside, after the agent stops, before the claim is accepted (C16's half outside) | anything that appeared during the run; **a container that exited under the measurement** — `docker run` carries `--rm`, so it leaves nothing behind | claim rejected, `indeterminate` |

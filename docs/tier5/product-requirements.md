---
status:        directional
owner:         human
enforcement:   review-cadence
evidence:      none — written pre-Phase-0 as a register stub (D32)
falsifies_if:  A requirement here is contradicted by what a named buyer actually asks for.
review_after:  Phase 0.75
---

# Product Requirements

**Status: stub.** This document exists to hold its place in the register, declare what
will enforce it, and state what would prove it wrong. It is deliberately not written
out: content written before the evidence exists cannot be current, and a wrong document
is worse than an absent one.

## Purpose

What the product must do for a named buyer, written from demand-gate conversations rather than from imagination. Directional until Phase 0.75 produces evidence.

## Enforcement

`review-cadence` — owned by `human`.

## Falsification condition

A requirement here is contradicted by what a named buyer actually asks for.

## Promotion

Promote this stub to full content when Phase Phase 0.75 can supply evidence for it.
On promotion, replace `evidence:` with what the content actually rests on.

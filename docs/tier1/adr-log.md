...
> `m2/containment-controls`. Numbering is sequential and never reused, so the draft takes
> the next free number rather than join the 0029 collision. The number is fixed by the
> operator's merge-order ruling at merge; if that ruling renumbers this record, the number
> and this note are corrected in the merge commit, per the `fa62b4b` precedent, before the
> record is published on `main`. The commits on this branch name the drafted number and
> cannot be rewritten; the corrected note is what reconciles a reader who follows them.
> The note travels with the record.

## ADR-0031 — Operator-plane memory is recall over the committed corpus, not a store
...
5. **Python in the repository; the npm package stays external** (D13). AgentDB is a
   ...
   `tools/`, carrying vaultgraph's status: local/manual, selftest present, not a CI gate,
   outside the protected set, O9 not applicable.
...
**A CI gate over the tool.** vaultgraph's status — local/manual, selftest present, not a
gate — is the standing this tool inherits, and invariant 5 fixes it. A CI job would be a

...
def test_the_register_yields_every_non_generated_document() -> None:
    # 72. docs/README.md and docs/READING-MAP.md are generated and excluded, exactly as
    ...
    result = _result()
    assert sum(1 for n in result.nodes if n.kind is NodeKind.DOCUMENT) == 72
...
def test_every_falsification_condition_in_the_corpus_is_data() -> None:
    # 72 in document frontmatter, 5 in decision cells. This is the relation the graph exists
...
def test_every_document_carries_a_falsification_condition() -> None:
    # The relation the graph exists to make visible. All 72 state one; a parser that stopped
